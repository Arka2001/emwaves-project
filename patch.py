...
def PatchFunction(thetaInDeg, phiInDeg, Freq, W, L, h, Er):
    ...
    Numtr2 = sin(k0 * heff * cos(phi) / 2)
    Demtr2 = (k0 * heff * cos(phi)) / 2
    Fphi = (Numtr2 / Demtr2) * cos((k0 * Leff / 2) * sin(phi))

    Numtr1 = sin((k0 * heff / 2) * sin(theta))
    Demtr1 = ((k0 * heff / 2) * sin(theta))
    Numtr1a = sin((k0 * Weff / 2) * cos(theta))
    Demtr1a = ((k0 * Weff / 2) * cos(theta))
